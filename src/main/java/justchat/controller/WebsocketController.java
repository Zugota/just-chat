...
import justchat.config.HttpInterceptor;
import justchat.model.*;
import justchat.repository.*;
...
public class WebsocketController implements WebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession wss) throws IOException {
        try {
            ...
            if ((session != null) && (session.getUser() != null)) {
                ...
                session.setOtherSideUsername(JustChatApplication.broadcastUsername);
                changePage(session);
                ...
                try {
                    if (x.getUser() != null) {
                        if (excludeUsername != null && x.getOtherSideUsername().equals(excludeUsername)) {
                            x.setOtherSideUsername(JustChatApplication.broadcastUsername);
                            changePage(x);
                        }
    ...
    public String createFileMessageUIComponent(Message msg, boolean self) throws IOException {
        String text = "";
    ...
    public String createTextMessageUIComponent(Message msg, boolean self) throws IOException {
        String text = "";
        Map<String, String> params = new HashMap<>();
        params.put("id", "" + msg.getId());
        params.put("date", "date=\"" + msg.getDate() + "\"");
    ...
    private String createUsersListUIComponent(String username, String activeUsername) throws IOException {
        ...
        User broadcastUser = users.stream().filter(x -> x.getUsername().equals(JustChatApplication.broadcastUsername))
                .collect(Collectors.toList()).get(0);
        users.remove(broadcastUser);
        users.remove(users.stream().filter(x -> x.getUsername().equals(username))
                .collect(Collectors.toList()).get(0));
    ...
    private void routePacket(String selfPack, String otherPack, Session currentSession) throws IOException {
        ...
        if (otherSideUsername.equals(JustChatApplication.broadcastUsername)) {
            for (User user : userRepository.findAll()) {
                String username = user.getUsername();
                if (username.equals(senderUsername) || username.equals(JustChatApplication.broadcastUsername)) {
                    continue;
                }
