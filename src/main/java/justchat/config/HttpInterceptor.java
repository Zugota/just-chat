...
public class HttpInterceptor implements HandlerInterceptor {
    ...
    public HttpInterceptor() {
        ...
        for (String filename : Objects.requireNonNull(new File(JustChatApplication.imageResourcePath).list())) {
            allowedFiles.add("/" + filename);
        }
        for (String filename : Objects.requireNonNull(new File(JustChatApplication.styleResourcePath).list())) {
            allowedFiles.add("/" + filename);
        }
        for (String filename : Objects.requireNonNull(new File(JustChatApplication.scriptResourcePath).list())) {
            allowedFiles.add("/" + filename);
        }
//        for (String filename : Objects.requireNonNull(new File(JustChatApplication.miscResourcePath).list())) {
//            allowedFiles.add("/" + filename);
//        }
        for (String filename : Objects.requireNonNull(new File(JustChatApplication.pageResourcePath).list())) {
    ...
    private void putHeaderParams(Map<String, String> params, User user) {
        if (user != null) {
            ...
            params.put("headerAccountText", "Войти / Зарегистрироваться");
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (allowedFiles.contains(uri)) {
            ...
            if (uriLC.endsWith(".svg")) {
                ...
                response.getOutputStream().write(byteUtils.readBytes(new File((JustChatApplication.imageResourcePath + "/" + uri)), false));
            } else if (uriLC.endsWith(".css")) {
                response.setContentType("text/css");
                response.getOutputStream().write(byteUtils.readBytes(new File((JustChatApplication.styleResourcePath + "/" + uri)), false));
            } else if (uriLC.endsWith(".json")) {
                response.setContentType("application/json");
                response.getOutputStream().write(byteUtils.readBytes(new File((JustChatApplication.miscResourcePath + "/" + uri)), false));
            } else if (uriLC.endsWith(".js")) {
                response.setContentType("text/javascript");
                response.getOutputStream().write(byteUtils.readBytes(new File((JustChatApplication.scriptResourcePath + "/" + uri)), false));
        ...
        if (uri.startsWith("/image-file-preview/")) {
            ...
            if (info.getImgPrevFileDataId() == null) {
                ...
                fileData = byteUtils.readBytes(new File(JustChatApplication.uploadPath + "/" + info.getFileDataId()), true);
            } else {
                mediaType = byteUtils.getMediaType("a." + JustChatApplication.imgThumbnailFormat);
                fileData = byteUtils.readBytes(new File(JustChatApplication.uploadPath + "/" + info.getImgPrevFileDataId()), true);
            }
